...
static int gpioled_open(struct inode *inode, struct file *filp)
{
    ...
    //原子操作第一种方法
    /*if(atomic_read(&ledlock) <= 0)  //判断原子变量是否小于等于0，是则表示资源已经被占用处于忙碌
    {
        return -EBUSY;
    }
    atomic_dec(&ledlock);  //资源未被占用则原子变量减一，表示本线程占用了led资源*/

    //原子操作第二种方法
    if(atomic_dec_and_test(&ledlock) == false)  //原子变量减一并判断是否为0，是则返回true，表明未被占用
    {
        atomic_inc(&ledlock);  //如果是false说明减一前值为0，是已经被占用了，重新加一回到0并返回错误
        return -EBUSY;
    }
...
static int gpioled_close(struct inode *inode, struct file *filp)
{
    ...
    atomic_inc(&ledlock);  //原子变量加一，表示本线程释放了led资源
    return 0;
}
...
{
    int ret = 0;
    atomic_set(&ledlock, 1); //原子变量初始化值为1，确保同一时间只有一个线程可以占用控制此led设备
